...
import multiprocessing, subprocess, os, argparse, time, datetime, socket, base64, threading, hashlib, binascii, signal, sys, getpass
    ...
    try:
        while True:
            with lock:
                ...
                for line in [_f for _f in process.communicate()[0].split('\n') if _f]:
                    share = line.strip('\t')
                ...
                for share in shares:
                    ...
                    if [item for item in [_f for _f in process.communicate()[0].split('\n') if _f] if "NT_STATUS" in item]: #if NT_STATUS in response => closed share/error
